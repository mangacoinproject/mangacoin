Mangacoin Core version 0.16.2 is now available from:
...
Mangacoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
Mangacoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Mangacoin Core is now 4.8. No effort will be
...
Mangacoinproject plans checkpoint distribution.
To deny checkpoint, use the argument `-cmdcheckpoint=false` with mangacoind or mangacoin-qt or add `cmdcheckpoint=false` to your mangacoin.conf. 
It is a function provided for the purpose of invalidating 51% attack on mangacoin, but it is not mandatory.
...
- mangacoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- mangacoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
